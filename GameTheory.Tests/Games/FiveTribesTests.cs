...
namespace GameTheory.Tests.Games
{
    ...
    public class FiveTribesTests
    {
        ...
        public void ComplexAssasinationScenario_Always_GoesAccordingToPlan()
        {
            ...
            var state = Transition(template, new GameState(
                ...
                lastDirection: template.LastDirection,
            ...
            state = MakeMove(state, playerA, m => Is.InstanceOf<PayMeeplesAndResourcesMove>().Matches(m));
            state = MakeMove(state, playerA, m => Is.InstanceOf<Sibittis.DrawDjinnsMove>().Matches(m));
            state = MakeMove(state, playerA, m => m.ToString() == "Take Ibus");
            state = MakeMove(state, playerA, m => state.MakeMove(m).GetAvailableMoves(playerA).Any(x => Is.InstanceOf<Ibus.DoubleAssassinKillCountMove>().Matches(x)));
            state = MakeMove(state, playerA, m => m.ToString() == "Double the number of meeples your Assassins kill this turn");
            Assert.That(state.GetAvailableMoves(), Has.None.InstanceOf<AssassinatePlayerMove>().With.Property("Victim").EqualTo(playerB));
            Assert.That(state.GetAvailableMoves(), Has.Some.InstanceOf<AssassinatePlayerMove>().With.Property("Victim").EqualTo(playerC));

            state = MakeMove(state, playerA, m => state.MakeMove(m).GetAvailableMoves(playerA).Any(x => Is.InstanceOf<AnunNak.AddMeeplesMove>().Matches(x)));
