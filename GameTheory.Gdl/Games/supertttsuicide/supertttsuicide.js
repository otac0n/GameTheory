...
function maketable (boardrow,boardcol,state)
 {var table = document.createElement('table');
  table.setAttribute('cellspacing','0');
  var current = compfindp(seq('currentBoard',boardrow,boardcol),state,seq());
  table.setAttribute('bgcolor','#ffffff');
  if (current) table.setAttribute('bgcolor','#cccccc');
  var states = [];
  for (var i = 0; i < state.length; i++) states.push(seq("true", state[i]));
  var winner = compfindx('Z',seq('win',boardrow,boardcol,'Z'),states,library);
  if (winner && winner[0] == "x") table.setAttribute("bgcolor", "#ff0000");
  if (winner && winner[0] == "o") table.setAttribute("bgcolor", "#0000ff");
...
function makecell (row,boardrow,boardcol,rownum,colnum,state)
 {var cell = row.insertCell(colnum);
  ...
  var mark = compfindx('Z',seq('mark',boardrow,boardcol,rownum,colnum,'Z'),state,seq());
