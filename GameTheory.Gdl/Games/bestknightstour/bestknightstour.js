...
function maketable (boardrow,boardcol,state)
 {var table = document.createElement('table');
  ...
  makerow(table,boardrow,boardcol,3,state);
  makerow(table,boardrow,boardcol,4,state);
  makerow(table,boardrow,boardcol,5,state);
  return table}
...
function makerow (table,boardrow,boardcol,rownum,state)
 {var row =table.insertRow(rownum);
  ...
  makecell(row,boardrow,boardcol,rownum,3,state);
  makecell(row,boardrow,boardcol,rownum,4,state);
  return row}
...
function makecell (row,boardrow,boardcol,rownum,colnum,state)
 {var cell = row.insertCell(colnum);
  ...
  if (mark=='knight') {cell.innerHTML = '<img src="/ggp/games/bestknightstour/White_Knight.png" width="36" height="36"/>'};
